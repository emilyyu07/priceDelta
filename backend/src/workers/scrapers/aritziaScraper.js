...
async function scrapeAritziaPrice(productUrl) {
    ...
    const scrapeTask = async () => {
        try {
            ...
            for (const selector of priceSelectors) {
                try {
                    ...
                    if ((await element.count()) > 0) {
                        ...
                        console.log(`✅ [Scraper] Found price using selector "${selector}": ${rawPriceText}`);
                        priceFound = true;
                        break;
                    }
                }
            ...
            if (!priceFound) {
                throw new Error("DOM Parse Error: Could not find price element. Aritzia may have changed their DOM structure.");
            }
            console.log(`💰 [Scraper] Successfully extracted raw price string: ${rawPriceText}`);
            ...
            try {
                ...
                if (!imageUrl) {
                    const productImage = await page
                        .locator('img[alt*="product"], img[data-testid="product-image"], .product-image img')
                        .first()
                        .getAttribute("src");
                    if (productImage) {
            ...
            console.error(`❌ [Scraper] Failed to scrape Aritzia: ${productUrl}`, error);
            // Check for specific error types
            if (error instanceof Error) {
                if (error.message.includes("timeout")) {
                    throw new Error("Scrape timeout - Aritzia might be slow or blocking us");
                }
