...
exports.sendWelcomeEmail = exports.sendPriceDropEmail = void 0;
const nodemailer_1 = __importDefault(require("nodemailer"));
const env_js_1 = require("./env.js");
// transporter (reusable connection pool to gmail)
const transporter = nodemailer_1.default.createTransport({
    host: env_js_1.env.SMTP_HOST, // smtp.gmail.com
    port: env_js_1.env.SMTP_PORT || 587,
    secure: false,
    auth: {
        user: env_js_1.env.SMTP_USER, // pricedeltanotif@gmail.com
        pass: env_js_1.env.SMTP_PASS, // app pw
    },
});
const sendPriceDropEmail = async (toEmail, productName, newPrice, productUrl) => {
    try {
        const info = await transporter.sendMail({
            from: `"PriceDelta Alerts" <${env_js_1.env.SMTP_USER}>`,
            to: toEmail,
...
exports.sendPriceDropEmail = sendPriceDropEmail;
const sendWelcomeEmail = async (toEmail, userName) => {
    try {
        const displayName = userName || toEmail.split("@")[0]; // Use name or email username
        const info = await transporter.sendMail({
            from: `"PriceDelta Team" <${env_js_1.env.SMTP_USER}>`,
            to: toEmail,
        ...
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
          ...
          <div style="text-align: center; margin-bottom: 25px;">
            <a href="${env_js_1.env.FRONTEND_URL}/dashboard" 
      ...
      `,
        ...
        console.log(`[Mailer] Welcome email sent successfully to ${toEmail}. Message ID: ${info.messageId}`);
    }
    catch (error) {
        console.error(`[Mailer] Failed to send welcome email to ${toEmail}:`, error);
    }
};
